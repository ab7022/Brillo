...
export default function Skills({ skill }) {
  ...
  const programming_technical_skillsObject = skill?.[0]?.programming_technical_skills;
  ...
  const programming_technical_skills = programming_technical_skillsObject
    ? programming_technical_skillsObject.split(",").map((skill: any) => skill.trim())
    : [];
          ...
          <ul className="flex flex-wrap justify-center gap-2 text-lg text-gray-800">
            {programming_technical_skills.map((skill, index) => (
              <motion.li
                ...
                key={index}
                variants={zoomInAnimationVariants}
                initial="initial"
                whileInView="animate"
                viewport={{ once: true }}
                custom={index}
